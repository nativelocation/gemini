CREATE OR REPLACE VIEW VW_ADMIN_SEC_USER AS
  ...
  WHERE A.STATUS = 1 AND B.IS_ACTIVE_IND = 1
        AND C.PARTY_TYPE_CD = 1 AND EXISTS(SELECT 1
                                           ...
                                           WHERE R.SEC_USER_ID = A.SEC_USER_ID);
...
CREATE OR REPLACE VIEW VW_ENROLLMENT_PROGRESS AS
  SELECT
    ...
    CITY_CODE,
    SCHOOL_ID,
    SUM(CASE WHEN TYPE IN ('REGULAR', 'SPECIALIZED_ALTERNATE_SCHOOLS', 'OCCUPATIONAL') AND ENTRY_TYPE = 'EXISTING'
      THEN 1
        ELSE 0 END)                     CONFIRMADOS,
    SUM(CASE WHEN TYPE IN ('REGULAR_ALTERNATE_SCHOOLS', 'SPECIALIZED_ALTERNATE_SCHOOLS', 'OCCUPATIONAL') AND
                  ENTRY_TYPE = 'NEW'
      THEN 1
        ELSE 0 END)                     NUEVO_INGRESO,
    SUM(CASE WHEN TYPE IN ('REGULAR_ALTERNATE_SCHOOLS') AND ENTRY_TYPE = 'EXISTING'
      THEN 1
        ELSE 0 END)                     RECHAZADOS
